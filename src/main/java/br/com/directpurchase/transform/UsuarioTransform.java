...
public class UsuarioTransform {
	...
	public UsuarioTransform(EntitysFetchDao entitysFetchDao, PessoaTransform pessoaTransform) {
	...
	public UsuarioDto transform(Usuario entity) {
		...
		List<CompradorDto> compradores = entity.getCompradores().stream().map(pessoaTransform::transform).toList();
		List<FornecedorDto> fornecedores = entity.getFornecedores().stream().map(pessoaTransform::transform).toList();
